...
do
  ...
  do
    ...
    if [[ -z $NORUN ]]; then
      float=$(./bin/inversek2j.out ${f} data/output/${filename}_${benchmark}_out.data $l12)
      mfloat=$(match_time "$float")
  
      fix=$(./bin/inversek2j.out.fixp ${f} data/output/${filename}_${benchmark}_out.data.fixp $l12)
      mfix=$(match_time "$fix")
    else
      mfloat='0'
      mfix='0'
    ...
    printf "$FORMAT" "${benchmark}_${filename}_0$i" $mfix $mfloat 0 0 "$mrel_error" "$mabs_error"
